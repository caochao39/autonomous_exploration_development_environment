...
def listener(args=None):
    ...
    timeDurationSub = node.create_subscription(Float32, '/time_duration', timeDurationCallback,5)
    # rospy.Subscriber("/runtime", Float32, runTimeCallback)
    runTimeSub = node.create_subscription(Float32, '/runtime', runTimeCallback,5)
    # rospy.Subscriber("/explored_volume", Float32, exploredVolumeCallback)
    exploredVolumeSub = node.create_subscription(Float32, '/explored_volume', exploredVolumeCallback,5)
    # rospy.Subscriber("/traveling_distance", Float32, travelingDistanceCallback)
    travellingDistanceSub = node.create_subscription(Float32, '/traveling_distance', travelingDistanceCallback,5)
